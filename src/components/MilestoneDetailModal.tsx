      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-3xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 sm:space-y-6 text-slate-800">
            ...
            {jalon.conditionsRemplies.length === 0 && jalon.conditionsManquantes.length === 0 ? (
              ...
              <div className="space-y-3">
                ...
                {jalon.conditionsRemplies.map((cond, idx) => (
                  <div key={`rempli-${idx}`} className="p-3 bg-emerald-50/60 border border-emerald-200 rounded-lg">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                        <span className="text-xs font-bold text-emerald-950">{cond.libelle}</span>
                      </div>
                    ...
                    <div className="mt-2 flex items-center justify-between text-xs text-slate-600">
                      <span>Acquis : <strong className="text-slate-800">{cond.valeurActuelle}</strong></span>
                      <span>Requis : <strong>{cond.valeurRequise}</strong></span>
                    </div>
                    ...
                    <p className="text-[11px] text-slate-600 mt-1.5">{cond.detailsExplicatifs}</p>
                  </div>
                ))}

                {/* Conditions manquantes ou en cours */}
                {jalon.conditionsManquantes.map((cond, idx) => (
                  <div key={`manque-${idx}`} className="p-3 bg-amber-50/70 border border-amber-200 rounded-lg">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-900">{cond.libelle}</span>
                      </div>
                    ...
                    <div className="mt-2 flex items-center justify-between text-xs text-slate-600">
                      <span>Actuel : <strong className="text-slate-800">{cond.valeurActuelle}</strong></span>
                      <span>Requis : <strong>{cond.valeurRequise}</strong></span>
                    </div>
                    ...
                    <p className="text-[11px] text-slate-700 mt-1.5">{cond.detailsExplicatifs}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              ...
              <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl">
                ...
                {jalon.justificatifsRequis.length === 0 ? (
                  <p className="text-slate-500 italic">Aucune démarche particulière pour l agent.</p>
                ) : (
              ...
              <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl">
                ...
                {jalon.decisionsAdministrativesRequises.length === 0 ? (
                  <p className="text-slate-500 italic">Aucun acte spécifique requis.</p>
                ) : (
                  <ul className="space-y-1.5">
          ...
          {jalon.hypothesesEtAlertes.length > 0 && (
            <div className="bg-slate-100/70 border border-slate-200 p-3.5 rounded-xl text-xs text-slate-700">
          ...
          <div className="pt-2 border-t border-slate-200">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="text-xs bg-slate-100 hover:bg-slate-200 text-slate-800 font-medium px-2.5 py-1.5 rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
              >
              ...
              {isContractuel ? (
                <>
                  <button
                    ...
                    className="text-xs bg-emerald-50 hover:bg-emerald-100 text-emerald-900 border border-emerald-200/90 font-bold px-2.5 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer shadow-2xs"
                  >
        ...
        <div className="p-3.5 sm:p-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <button
            onClick={onClose}
            className="w-full sm:w-auto text-xs font-semibold bg-slate-800 hover:bg-slate-900 text-white px-5 py-2.5 rounded-xl transition-colors cursor-pointer"
          >
            Fermer la fiche
          </button>
        </div>
