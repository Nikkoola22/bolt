    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 sm:p-6">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-8 overflow-y-auto space-y-6 text-slate-900 bg-white">
          ...
          <div className="border border-slate-200 rounded-xl p-4 bg-slate-50/50">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
              <div>
                ...
                <span className="font-bold text-slate-900">{profil.prenom}{profil.nom ? ` ${profil.nom}` : ""}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Cadre & Grade :</span>
                <span className="font-bold text-slate-900">{grade.nom}</span>
                <span className="text-[10px] text-slate-500 block">(Catégorie {grade.categorie})</span>
              </div>
              <div>
                <span className="text-slate-500 block">Échelon & Indice :</span>
                <span className="font-bold text-slate-900">{profil.echelonActuel}e échelon</span>
                <span className="text-slate-700 block">IM {jalonActuel.indiceMajore} (IB {jalonActuel.indiceBrut})</span>
              </div>
              <div>
                <span className="text-slate-500 block">Quotité de travail :</span>
                <span className="font-bold text-slate-900">{profil.quotiteActuelle}%</span>
                <span className="text-[10px] text-slate-500 block">Depuis le {formatDateFrench(profil.dateEffetEchelonActuel)}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div className="bg-purple-50/60 border border-purple-200 p-3 rounded-xl">
                <div className="font-bold text-purple-950 flex items-center gap-1 mb-1">
                ...
                <div className="text-purple-800 font-medium mt-1">
                      ...
                      <tr key={evt.id}>
                        ...
                        <td className="py-2 px-3">
                          {evt.impacteAvancementEchelon ? (
                            <span className="text-rose-700 font-bold">Décalage de {evt.dureeMois} mois</span>
                          ) : (
                            <span className="text-emerald-700 font-bold">Maintien à 100%</span>
                          )}
                        </td>
          ...
          <div className="pt-6 border-t border-slate-300 grid grid-cols-2 gap-8 text-xs">
            <div className="border border-slate-300 rounded-xl p-4 h-28 flex flex-col justify-between">
              <span className="font-semibold text-slate-700">Signature de l agent :</span>
              <span className="text-[10px] text-slate-400">Date et signature</span>
            </div>
            <div className="border border-slate-300 rounded-xl p-4 h-28 flex flex-col justify-between">
              <span className="font-semibold text-slate-700">Avis du supérieur hiérarchique direct :</span>
              <span className="text-[10px] text-slate-400">Date, visa et observations éventuelles</span>
            </div>
          </div>
        ...
        <div className="p-4 bg-slate-100 border-t border-slate-200 flex items-center justify-between print:hidden">
          ...
          <button
            onClick={onClose}
            className="text-xs font-semibold bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-lg cursor-pointer"
          >
            Fermer
          </button>
        </div>
